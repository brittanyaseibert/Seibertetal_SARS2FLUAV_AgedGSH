---
title: "Dada2_HamsterSARS2_Intestine_Analysis"
...
This file, similar to the Dada2_HamsterSARS2_Lung_Analysis, will go through the DADA2 analysis used to filter, trim, remove chimeras, and combine 2 different batches used to sequence the intestine samples in the microbiome analysis. Two batches were needed since some of the sequences were very low quality from the first sequencing run; therefore, we ran a second 
...
```{r}
# Forward Reads
pdf(file="plot_F-1_67_Intestine.pdf", height=6, width=10)
plotQualityProfile(fnFs_67[1:12])
dev.off()

pdf(file="plot_F-1_68_Intestine.pdf", height=6, width=10)
plotQualityProfile(fnFs_68[1:12])
dev.off()

# Reverse Reads
pdf(file="plot_R-1_67_Intestine.pdf", height=6, width=10)
plotQualityProfile(fnRs_67[1:12])
dev.off()

pdf(file="plot_R-1_68_Intestine.pdf", height=6, width=10)
...
write.table(x = out_67,file = "/path/outFiltered_67_Intestine.csv", sep = ',', row.names = TRUE, col.names = TRUE)

write.table(x = out_68,file = "/path/outFiltered_68_Intestine.csv", sep = ',', row.names = TRUE, col.names = TRUE)
```
...
# Forward Reads
pdf(file="/path/TrainingDataF_67_Intestine.pdf", height=8, width=12)
plotErrors(errF_67, nominalQ=TRUE)
dev.off()
# Reverse Reads
pdf(file="/path/TrainingDataR_67_Intestine.pdf", height=8, width=12)
plotErrors(errR_67, nominalQ=TRUE)
dev.off()

# Forward Reads
pdf(file="/path/TrainingDataF_68_Intestine.pdf", height=8, width=12)
plotErrors(errF_68, nominalQ=TRUE)
dev.off()
# Reverse Reads
pdf(file="/home/path/TrainingDataR_68_Intestine.pdf", height=8, width=12)
...
head(track_67)
write.table(x = track_67, file = "/path/importOverviewTable_67_Intestine.csv", sep = ',', row.names = TRUE, col.names = TRUE)
...
head(track_68)
write.table(x = track_68,file = "/path/importOverviewTable_68_Intestine.csv", sep = ',', row.names = TRUE, col.names = TRUE)
...
head(track_complete)
write.table(x = track_complete, file = "/path/nochimeras_allSamples_Intestine.csv", sep = ',', row.names = TRUE, col.names = TRUE)
```
...
write.table(asv_tab, "ASVs_counts_Intestine.tsv", sep="\t", quote=F, col.names=NA)

# Exporting taxonomy table:
asv_tax_dada <- taxa
rownames(asv_tax_dada) <- gsub(pattern=">", replacement="", x=asv_headers)
write.table(asv_tax_dada, "ASVs_taxonomy_dada_Intestine.tsv", sep = "\t", quote=F, col.names=NA)

asv_tax_dada_wSpecies <- taxawithSpecies
rownames(asv_tax_dada_wSpecies) <- gsub(pattern=">", replacement="", x=asv_headers)
write.table(asv_tax_dada_wSpecies, "ASVs_taxonomy_dada_wSpecies_Intestine.tsv", sep = "\t", quote=F, col.names=NA)
```
